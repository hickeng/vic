...
func (handler *ContainersHandlersImpl) ContainerWaitHandler(params containers.ContainerWaitParams) middleware.Responder {
	...
	case <-c.WaitForState(exec.StateStopped):
		// Populate this from a copy of the container so we don't block subsequent updates
		// TODO: refactor convertContainerToContainerInfo so that the container it operates off can be copied under lock
